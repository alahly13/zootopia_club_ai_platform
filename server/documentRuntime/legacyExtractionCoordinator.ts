...
import { legacyFileTypeDetectionService } from './legacyFileTypeDetectionService.js';
...
type LegacyExtractionCoordinatorInput = {
  ...
  reportStage?: (input: {
    stage: Extract<DocumentOperationState['stage'], 'extracting' | 'finalizing_extraction'>;
    message: string;
  }) => Promise<void> | void;
};
...
/**
 * Legacy layered extraction path kept available for controlled recovery.
 *
 * Keep this isolated behind backend-only engine selection so the runtime never
 * fans out into multiple active extraction engines for a single intake job.
 */
export class LegacyExtractionCoordinator {
  async extract(input: LegacyExtractionCoordinatorInput): Promise<ExtractedArtifactEnvelope> {
    await input.reportStage?.({
      stage: 'extracting',
      message: 'Running legacy layered extraction',
    });

    const strategy = await legacyFileTypeDetectionService.resolveStrategy({
    ...
    await input.reportStage?.({
      ...
      message: 'Finalizing legacy extraction artifacts',
    });
    ...
    const fallbackChain = [
      ...
      shouldRunLatePdfOcrFallback
        ? `ocr:late-pdf-weak-pages:${weakPdfPages.join(',')}`
        : 'ocr:late-pdf-weak-pages:none',
    ...
    const payload = documentStructureNormalizer.normalize({
      artifactBase: {
        ...
        paths: {
          ...
          ocrBlocksPath:
            strategy.ocrPreferred || merged.ocrBlocks.length > 0
    ...
    logDiagnostic('info', 'document_runtime.extraction_completed', {
      ...
      details: {
        ...
        weakPdfPages,
        ocrUsed:
          Boolean(payload.ocrBlocks.length) ||
          payload.pageSegments.some((segment) => segment.kind === 'ocr' || segment.kind === 'hybrid'),
        warnings: pipelineWarnings,
        extractionEngine: 'python_legacy',
      },
    });
...
export const legacyExtractionCoordinator = new LegacyExtractionCoordinator();
