...
import { RefreshCw, Search, ShieldCheck, Clock, Hash, Coins, CheckCircle2, AlertTriangle, Download } from 'lucide-react';
import { auth } from '../../firebase';
import { cn } from '../../utils';

type FastAccessCreditEvent = {
  id: string;
  userId: string;
  operationId: string;
  traceId: string;
...
export const FastAccessCreditAuditView: React.FC = () => {
  const today = new Date();
  const sevenDaysAgo = new Date(Date.now() - 6 * 24 * 60 * 60 * 1000);

  const [events, setEvents] = useState<FastAccessCreditEvent[]>([]);
  ...
  const fetchEvents = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/fast-access/credit-events?${params.toString()}`, {
        headers: {
      ...
      if (!response.ok || !payload?.success) {
        throw new Error(payload?.error || 'Failed to load fast-access credit events.');
      }
  ...
  const handleCsvExport = () => {
    ...
    anchor.href = url;
    anchor.download = `fast-access-credit-audit-${fromDate || 'all'}-to-${toDate || 'all'}.csv`;
    ...
    <div className="space-y-6">
      <div className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-sm border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Faculty Fast Access</p>
            <h3 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-wider mt-1">Credit Events Audit</h3>
            <p className="text-xs text-zinc-500 mt-2">Compact trace of temporary credit deductions with operation and trace identifiers.</p>
          </div>
                ...
                <tr key={event.id} className="hover:bg-zinc-50/60 dark:hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs">
                    <div className="inline-flex items-center gap-2 px-2 py-1 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-200 font-bold">
                      <ShieldCheck size={12} /> {short(event.userId, 14)}
                    </div>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-sm text-zinc-500 italic">
                    {loading ? 'Loading events...' : 'No fast-access credit events found for the current filter.'}
                  </td>
                </tr>
              )}
            </tbody>
