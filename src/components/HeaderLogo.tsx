...
export const HeaderLogo = ({ className, iconClassName = "w-8 h-8", shortTextOnMobile = false }: { className?: string, iconClassName?: string, shortTextOnMobile?: boolean }) => (
  <div className={cn("flex items-center gap-2 shrink-0", className)}>
    ...
    <div className="flex flex-col">
      <span className={cn(
        "font-black text-xl tracking-tighter text-zinc-900 dark:text-white whitespace-nowrap leading-none",
        shortTextOnMobile ? "block" : "hidden sm:block"
      )}>
        {shortTextOnMobile ? (
          <>
            <span className="hidden lg:inline text-white">ZOOTOPIA</span>
            <span className="lg:hidden text-white">Z</span>
          ...
          <><span className="text-white">ZOOTOPIA</span><span className="text-emerald-500">CLUB</span></>
        )}
      </span>
